...
import com.neu.trend.databinding.DialogAlertBinding
import com.neu.trend.extensions.underlineText
...
class AlertDialog : BaseDialogFragment() {
    ...
    companion object {
        ...
        private const val EXTRA_DIALOG_CONFIRMATION_BUTTON = "EXTRA_DIALOG_CONFIRMATION_BUTTON"
        ...
        fun newInstance(
            ...
            AlertDialog().apply {
                arguments = Bundle().apply {
                    ...
                    putString(EXTRA_DIALOG_CONFIRMATION_BUTTON, confirmationButton)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            it.getString(EXTRA_DIALOG_TITLE)?.let { title -> binding.tvTitle.text = title }
            it.getString(EXTRA_DIALOG_MESSAGE)?.let { message -> binding.tvMessage.text = message }

            it.getString(EXTRA_DIALOG_CONFIRMATION_BUTTON)?.let { confirm -> binding.btnConfirm.text = confirm }
            it.getString(EXTRA_DIALOG_CANCEL_BUTTON)?.let { cancel -> binding.btnCancel.text = cancel }

            binding.btnConfirm.setOnClickListener { onConfirmationButtonClicked() }
            binding.btnCancel.underlineText()
    ...
    private fun onConfirmationButtonClicked() {
        if (activity is PopupDialogListener) {
            arguments?.let {
                (activity as PopupDialogListener).onDialogOk(
                    it.getString(EXTRA_DIALOG_CALLER_TAG)!!,
                    it.getString(EXTRA_DIALOG_TAG)!!
                )
            }
        }
        dismiss()
    ...
    private fun dismissDialog() {
        if (activity is PopupDialogListener) {
            arguments?.let {
                (activity as PopupDialogListener).onDialogClose(it.getString(EXTRA_DIALOG_TAG)!!)
            }
        }
        dismiss()
    }
